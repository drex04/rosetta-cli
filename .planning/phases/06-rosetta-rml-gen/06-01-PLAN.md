...
8. **`test_cli_json_array_input_exits_1`** — write `[]` to tmp JSON, invoke CLI, assert exit code 1 and stderr contains "must be a JSON object"

9. **`test_cli_invalid_decision_type_exits_1`** — decisions JSON with `{"multiplier": "not-a-number"}` → exit code 1 (pydantic ValidationError caught)

## Review Truths

<!-- [review] findings applied during plan-review 2026-04-13 -->
- `[review]` bare `import rdflib` removed from rml_builder.py — ruff F401 would fail quality gate
- `[review]` pydantic.ValidationError from MappingDecision(**props) is rescued with user-visible error message and exit code 1
- `[review]` JSON array input to decisions file is checked with isinstance(raw, dict) and exits 1 with clear message
- `[review]` README must NOT document `conversion_fn` until Plan 02 ships FnML — MappingDecision uses `fnml_function`; pydantic silently ignores unknown fields (no extra="forbid"), so a user passing `conversion_fn` gets exit 0 but wrong output (no unit conversion block). Remove `conversion_fn` from README example.
- `[review]` README shows `--source-format TEXT` but Click type is `choice([json|csv])` — fix README to show correct type hint.
- `[review]` `rr:subjectMap` template hardcodes `{id}` as the subject field — source data must have an `id` column/key for RMLMapper to generate subject IRIs. Document this convention in README.
- `[review]` `URIRef(decision.target_uri)` accepts any string without validation — malformed target_uri produces invalid but serializable Turtle. Deferred: URIs come from approved decisions, which SHACL validation should catch upstream.
...
All 9 tests pass, ruff clean, basedpyright clean.
