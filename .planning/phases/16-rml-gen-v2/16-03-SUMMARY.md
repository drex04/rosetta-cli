...
**[Known gap, accepted]** Numeric unit conversion in E2E: `transform_builder.build_slot_derivation` (16-01) still leaves `unit_conversion=None`; the fork only emits GREL when it's present. The E2E asserts **passthrough** values via compaction-tolerant key lookup + `pytest.approx(rel=1e-2)`. Structural truth #3 is satisfied (linear-convertible slot emitted in JSON-LD); numeric-value truth #3 is deferred. Flagged in STATE.md "Known gap" section.
