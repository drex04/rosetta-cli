...
uv run rosetta ingest    fixtures/nor_radar.csv --output nor_radar.linkml.yaml
uv run rosetta translate nor_radar.linkml.yaml  --output nor_radar_en.linkml.yaml --source-lang auto

uv run rosetta embed     nor_radar.linkml.yaml    --output nor_radar_nb.emb.json
uv run rosetta embed     nor_radar_en.linkml.yaml --output nor_radar_en.emb.json

# Master side
uv run rosetta ingest    fixtures/master_cop.ttl -f rdfs --output master_cop.linkml.yaml
uv run rosetta translate master_cop.linkml.yaml  --output master_cop_en.linkml.yaml --source-lang EN  # no-op
uv run rosetta embed     master_cop_en.linkml.yaml --output master_cop.emb.json
...
```bash
uv run rosetta translate eng_schema.linkml.yaml --output eng_schema_en.linkml.yaml --source-lang EN
```
