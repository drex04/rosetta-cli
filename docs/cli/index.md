...
- **Input** — read from a positional argument or stdin where sensible.
...
- **Config** — every default is overridable via `rosetta.toml` (`-c`/`--config` to specify a non-default path) or `ROSETTA_<SECTION>_<KEY>` env vars, with CLI flags winning. See [Configuration](../configuration.md).
