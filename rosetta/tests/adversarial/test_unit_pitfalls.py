...
Test 4 is the integration bridge: unit-detect → ledger append --dry-run surfaces
the 'recognized but unmapped' dBm diagnostic as a finding in LintReport.
...
def test_lint_surfaces_recognized_but_unmapped_unit(tmp_path: Path) -> None:
    """ledger append --dry-run emits a finding for a 'recognized but unmapped' dBm field.
