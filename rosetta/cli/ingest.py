...
def cli(
    ...
    try:
        ...
        if fmt is not None and len(schema_files) > 1:
            # --schema-format cannot apply uniformly when auto-detected formats differ
...
def _scaffold_rosetta_toml(master_out_path: Path) -> None:
    ...
    abs_master = master_out_path.resolve()
    abs_shacl = abs_master.with_suffix("").with_suffix(".shacl.ttl")
    content = (
